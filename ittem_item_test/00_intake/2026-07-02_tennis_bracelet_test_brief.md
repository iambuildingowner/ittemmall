...
- 게시 보류 이유:
  ...
  - 사용자 스크린샷 확인 결과 `잇템몰` Facebook 페이지는 없던 것이 아니라 `MICORE, 미코어` 그룹 아래에 있었다.
  - 확인된 잇템몰 Facebook 페이지: `잇템몰`, ID `1069905426214093`
  - 잇템몰 Meta 광고는 `잇템몰` Facebook 페이지 기준으로 집행해야 하므로, `Autorator` 상태로는 게시하지 않는다.
- 재개 조건:
  - Meta 광고 관리자 Facebook 페이지 드롭다운에서 `MICORE, 미코어` 그룹을 펼친 뒤 `잇템몰` 페이지를 선택하고 검토 후 게시한다.
