...
- 아이템 전용 메인 URL: `https://ittemmall.com/tennis-bracelet/`
...
- ICEBALL 공식 상품 상세 기준으로는 `LAB Diamond`가 아니라 `ICE CARAT` 표기를 사용한다. 이번 잇템몰 상품은 랩다이아가 아니라 5A 지르코니아/실버톤 패션 주얼리 포지션으로 둔다.
...
- 페이지 구조: `/tennis-bracelet/`는 테니스 팔찌 아이템의 전용 메인 페이지다. 이 페이지 안에서 상품 CTA를 누르면 주문/상세 하위 흐름으로 이어지게 본다.
...
- 사이즈 설명 이미지는 Codex가 SVG/HTML 도형으로 직접 그리지 않고, 이미지 생성 도구로 만든다. 숫자/표/주의 문구는 HTML 텍스트와 표로 분리한다.
- 대표 이미지: 착용컷
- 상세 이미지: 제품 단독컷, 잠금/스톤 디테일컷
- 대표 컬러: `#87CEEB`. 이번 테니스 팔찌 전용 페이지는 블랙 포인트가 아니라 이 색을 기준으로 진행한다. 특정 브랜드 로고/상표/포장 오인은 피한다.
...
- 추가 확인 시각: 2026-07-02 21:37 KST
- 상태: 게시 요청 후 Meta 광고 관리자에서 `처리 중` 표시. 아직 `검토 중` 또는 `게재 중` 최종 상태는 확인 전.
- 캠페인: `잇템몰_아이템검증`
- 광고 세트: `테니스팔찌_KR_F25-54_데일리주얼리_9980`
- 광고 세트 설정:
  - 지역: 대한민국
...
- 광고 초안:
  ...
  - 랜딩 URL: `https://ittemmall.com/tennis-bracelet/?utm_source=meta&utm_medium=paid_social&utm_campaign={{campaign.name}}&utm_content={{ad.name}}&utm_term={{adset.name}}`
- 게시 진행 / 보류 메모:
  - Facebook 페이지는 `잇템몰`로 수정 확인했다.
  - 검토 화면에서 `Facebook 페이지 잇템몰`, 랜딩 URL `https://ittemmall.com/tennis-bracelet/...`, 광고명 `아쿠아글로우테니스팔찌_착용컷_49900`을 확인했다.
  - `동의하고 게시`를 시도했지만 Meta 광고 관리자 목록으로 돌아가며 계정 전체 `검토 후 게시 (7개)`가 남았다.
  - 해당 검토 모달에는 잇템몰 테니스 팔찌만이 아니라 다른 프로젝트 초안 오류가 섞여 있었다. 확인된 오류 초안명: `웹사이트 문의_금액고시_클릭`, `골드레터 2`, `카카듀`, `Post: "Just $0.20 to Reach a Company"`.
  - 이 오류 초안들은 잇템몰 상품과 무관해 임의 삭제/수정하지 않았다.
  - `작업` 탭에서 필터를 지운 뒤 현재 선택된 테니스 팔찌 광고 1개가 `처리 중`으로 표시되는 것을 확인했다.
  - 2회 이상 새로고침해도 `처리 중`이 유지되어 Meta 내부 처리 대기 상태로 본다.
- 재개 조건:
  - Meta 처리 완료 후 상태가 `검토 중`, `게재 가능`, `게재 중`, `오류` 중 어디로 바뀌는지 다시 확인한다.
  - 계속 계정 전체 `검토 후 게시 (7개)`만 남아 있으면, 잇템몰이 아닌 오류 초안을 삭제/수정할지 별도 판단한다.
